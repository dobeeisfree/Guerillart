jQuery(document).ready ->
  ...
  $('#already_registered').click ->
    $('.ui.basic.modal.nav#signup').modal('hide');
    setTimeout((->
      setTimeout $('.ui.basic.modal.nav#signin').modal('show'), delay
      ), delay)


  # 회원가입 모달 닫았을 시 모달 초기화
  $('.ui.basic.modal.nav#signup').modal({
      onHide: ->
  ...
  # 비밀번호 찾기 모달 닫았을 시
  $('.ui.basic.modal.nav#forgot').modal({
      onHide: ->
        setTimeout((->
          setTimeout $('.ui.basic.modal.nav#signin').modal('show'), delay
        ), delay)
  ...
  $('#not_registered').click ->
    $('.ui.basic.modal.nav#signin').modal('hide');
    setTimeout((->
      setTimeout $('.ui.basic.modal.nav#signup').modal('show'), delay
    ), delay)
  ...
  $('#forgot_password').click ->
    $('.ui.basic.modal.nav#signin').modal('hide');
    setTimeout((->
      setTimeout $('.ui.basic.modal.nav#forgot').modal('show'). delay
    ), delay)

  # 로그인 모달 닫았을 시 모달 초기화
  $('.ui.basic.modal.nav#signin').modal({
      onHide: ->
